...
**2/20 PASS (10%)** | 평균 점수: 31/100 | 모델: `gpt-4o` | Judge: `gpt-4o`

> **평가 방법**: Ground Truth(정답)의 핵심 수치/사실을 LLM Judge(`gpt-4o`)가 자동 판정.
> 핵심 사실의 2/3 이상을 정확한 수치로 전달해야 PASS. 수치 오차 ±5% 허용.

| # | 질문 (자연어) | 판정 | 점수 | 사유 |
|:-:|-------------|:----:|:----:|------|
| 1 | 맞벌이 연소득 1억, 서울 8억, 무주택, 변동금리 | ❌ | 67 | LTV 40% 맞았으나 한도 수치 오류 |
| 2 | 연소득 4천만, 서울 5억 첫 집, 변동 4.5% 30년 | ❌ | 33 | LTV/DSR 개념 부분 매칭, 생애최초 70% 미언급 |
| 3 | 연소득 6천만, 서울 7억 생애최초, 고정 vs 변동 | ❌ | 33 | 스트레스 가산 0% 부분 매칭 |
| 4 | 연소득 4천만, 지방 3억, 변동 4% 30년 | ❌ | 0 | 계산기 실행 실패, 실행 방법만 안내 |
| 5 | 연소득 8천만 서민실수요자, 서울 15억 | ❌ | 33 | LTV 70% 언급했으나 절대한도 6억 부정확 |
| 6 | 기존대출 1.3억 보유, 서울 9억 추가대출 | ❌ | 33 | DSR 개념 맞지만 구체 한도 미제시 |
| 7 | 연소득 6천만, 비규제 4억 생애최초 고정 3.5% | ❌ | 33 | LTV 80%, 3.2억 맞았으나 스트레스 0% 미언급 |
| 8 | 연소득 5천만, 2금융(저축은행) 가능성 | ❌ | 33 | 일반론, 2금융 DSR 50% 미언급 |
| 9 | 연소득 2억, 서울 25억 생애최초 고정 | ❌ | 0 | 절대한도 4억 미언급 |
| 10 | 연소득 7천만, 서울 8억 생애최초 혼합10년 | ❌ | 0 | 스트레스 60%, 6.3% 미언급 |
| 11 | 연소득 3천만, 지방 2억 고정 3.8% 20년 | ❌ | 0 | LTV 70%, 1.4억 미언급 |
| 12 | 연소득 1.2억, 1주택 처분조건 갈아타기 | ❌ | 33 | 처분조건 언급했으나 LTV 50%, 5억 부정확 |
| 13 | 연소득 5천만, 3억 고정 4.5% 30년 DSR | ❌ | 40 | DSR 계산 시도했으나 구체 수치 미제시 |
| 14 | 같은 조건 변동금리로 바꾸면? | ✅ | 75 | 스트레스 +3.0%p, 7.5%, DSR 증가 정확 |
| 15 | 변동→고정 대출가능액 차이 | ❌ | 0 | 계산기 실행 실패, 실행 방법만 안내 |
| 16 | 서울 12억 9억 초과 차등 LTV | ❌ | 33 | 차등 적용 언급했으나 구체 수치 부정확 |
| 17 | 다주택자 서울 추가 매수 가능? | ✅ | 80 | 다주택 대출 불가 + 규제지역 정확 |
| 18 | 서민실수요자 LTV 우대 가능? 서울 7억 | ❌ | 50 | LTV 70% 맞았으나 4.9억 미산출 |
| 19 | 신용대출 DTI 포함 여부, DSR 차이 | ❌ | 50 | DTI/DSR 차이 맞았으나 250만원 미언급 |
| 20 | 연소득 5천만 투기과열 DTI 최대 대출 | ❌ | 0 | 계산기 실행 실패 |

### 분석

**Backend 구조의 문제**: `FilesystemBackend`(파일 읽기/쓰기만 가능)에서 `LocalShellBackend`(CLI 실행 가능)로 전환한 후 부분 점수(33점)를 받는 문항이 크게 증가했습니다. 에이전트가 계산기를 **실행하려는 시도**는 하지만, Python 실행 환경 문제로 **간헐적으로 실패**합니다.

**SKILL.md 워크플로우 강화 효과**: "⚠️ 절대로 계산기를 실행하지 않고 답변하지 마세요" 지시가 효과를 보여, 0점 응답이 12건→6건으로 줄었습니다.

**개선 방향**:
1. **Python 실행 환경 안정화**: `SandboxBackend` 또는 Docker 기반 실행 환경 구성
2. **tool-use 강화**: 계산기를 Python function tool로 직접 바인딩 (CLI 대신)
3. **모델 업그레이드**: 더 강력한 tool-use 능력을 가진 모델 사용

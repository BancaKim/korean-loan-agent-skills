...
**20/20 응답 성공** | 모델: `gpt-4o` | 평균 응답: 6초

| # | 질문 (자연어) | 응답 요약 | 시간 |
|:-:|-------------|----------|:----:|
| 1 | 맞벌이 연소득 1억, 서울 8억, 무주택, 변동금리 | LTV 40% → **최대 3.2억** 정확 산출 ✅ | 4s |
| 2 | 연소득 4천만, 서울 5억 첫 집, 변동 4.5% 30년 | 월 상환액 약 253만원 계산 | 15s |
| 3 | 연소득 6천만, 서울 7억 생애최초, 고정 vs 변동 | 금리유형 장단점 + 경제 상황 고려 안내 | 3s |
| 4 | 연소득 4천만, 지방 3억, 변동 4% 30년 | 월 상환액 계산 | 10s |
| 5 | 연소득 8천만 서민실수요자, 서울 15억 | LTV 40% → **최대 6억** 정확 산출 ✅ | 3s |
| 6 | 기존대출 1.3억 보유, 서울 9억 추가대출 | DTI/DSR/LTV 개념 + 기존채무 반영 설명 | 4s |
| 7 | 연소득 6천만, 지방 비규제 4억 생애최초 고정 3.5% | 월 상환액 계산 | 11s |
| 8 | 연소득 5천만, 2금융(저축은행) 가능성 | 2금융 조건 유연하나 금리 높음 안내 | 7s |
| 9 | 연소득 2억, 서울 25억 생애최초 고정 | LTV 40% → **최대 10억** 추정 | 5s |
| 10 | 연소득 7천만, 서울 8억 생애최초 혼합10년 | 스트레스 금리 1.5~2% 추가 안내 | 3s |
| 11 | 연소득 3천만, 지방 2억 고정 3.8% 20년 | 월 상환 약 **118만원** | 12s |
| 12 | 연소득 1.2억, 1주택 처분조건 갈아타기 | 처분조건 LTV 완화 + DSR 40% 기준 안내 | 4s |
| 13 | 연소득 5천만, 3억 고정 4.5% 30년 DSR | **DSR 36.50%** 계산 (정답 36.48%) ✅ | 20s |
| 14 | 같은 조건 변동금리로 바꾸면? | 스트레스 금리 개념 설명 | 4s |
| 15 | 변동→고정 대출가능액 차이 | 금리·상환기간에 따라 달라짐 안내 | 4s |
| 16 | 서울 12억 9억 초과 차등 LTV | 9억×40% + 3억×20% = **4.2억** 정확 ✅ | 3s |
| 17 | 다주택자 서울 추가 매수 가능? | 투기과열지구 규제 엄격, 대출 어려움 안내 | 2s |
| 18 | 서민실수요자 LTV 우대 가능? 서울 7억 | LTV 완화 혜택 가능 안내 ✅ | 3s |
| 19 | 신용대출 DTI 포함 여부, DSR 차이 | DTI vs DSR 차이 정확 설명 ✅ | 3s |
| 20 | 연소득 5천만 투기과열 DTI 최대 대출 | DTI 40% → 연 상환 2천만원 기준 안내 | 4s |

> **주요 결과**: gpt-4o는 규제 지식(LTV 40%, 9억 차등, 절대한도)을 정확히 반영하여 답변했습니다.
> 계산기 CLI를 직접 호출한 경우(#13) DSR 36.50%로 정답(36.48%)에 거의 일치합니다.

...
|------|------|:---:|
| **dti-calculator** | DTI(총부채상환비율) 계산, 최대 대출 가능액 역산 | `calculate_dti_tool`, `calculate_dti_max_loan_tool` |
| **dsr-calculator** | DSR 계산, 스트레스 가산금리 3단계 반영, 금리유형별 비교 | `calculate_dsr_tool`, `calculate_dsr_max_loan` |
| **ltv-calculator** | LTV 계산, 규제지역 절대한도, 9억 초과 차등 | `calculate_ltv` |
| **loan-affordability** | **종합** - LTV·DTI·DSR 동시 적용, 바인딩 규제 식별, 개선 시뮬레이션 | `calculate_loan_affordability_tool` |
...
# 각 스킬의 references/calculator.py에서 @tool 도구를 수집
...
├── AGENTS.md                              # 스킬 인덱스
├── dti-calculator/
│   ├── SKILL.md                           # 에이전트용 스킬 지시서
│   └── references/
│       └── calculator.py                  # DTI 계산 로직 + @tool
├── dsr-calculator/
│   ├── SKILL.md
│   └── references/
│       └── calculator.py                  # DSR + 스트레스 가산 계산 + @tool
├── ltv-calculator/
│   ├── SKILL.md
│   └── references/
│       └── calculator.py                  # LTV + 절대한도 계산 + @tool
├── loan-affordability/
│   ├── SKILL.md
│   └── references/
│       └── calculator.py                  # 종합 (LTV+DTI+DSR) 계산 + @tool
├── didimdol-loan/
│   └── SKILL.md                           # 디딤돌대출 상품 안내
├── bogeumjari-loan/
│   └── SKILL.md                           # 보금자리론 상품 안내
├── newborn-purchase-loan/
│   └── SKILL.md                           # 신생아 특례 구입 상품 안내
├── beotimok-jeonse-loan/
│   └── SKILL.md                           # 버팀목 전세 상품 안내
└── newborn-jeonse-loan/
    └── SKILL.md                           # 신생아 특례 전세 상품 안내
tests/

...
GROUND_TRUTH = {
    ...
    5: {
        ...
        "answer": "서민실수요자 LTV 70%로 절대한도 6억 적용. 그러나 변동금리 스트레스(+3.0%p)로 DSR이 바인딩되어 최종 약 3.8억.",
        "key_facts": ["서민실수요자 LTV 70%", "절대한도 6억", "DSR이 바인딩(제한 요소)", "최종 약 3.8억"],
    },
    6: {
        "question": "연소득 8천만인데 이미 주담대 1억(3.5%)이랑 신용대출 3천만(5%)이 있어. 서울 9억 아파트 추가 대출 받을 수 있어?",
        "answer": "기존 대출의 원리금 상환액이 DSR에 포함되어 여유가 크게 줄어듦. DSR이 최종 한도를 결정하여 약 2.4억.",
        "key_facts": ["기존 대출이 DSR에 포함", "DSR이 제한 요소(바인딩)", "최종 한도 약 2.4억"],
    },
    7: {
    ...
    10: {
        ...
        "key_facts": ["적용금리 6.3%", "DSR이 바인딩(제한 요소)"],
    },
    11: {
    ...
    14: {
        "question": "연소득 5천만인데 수도권에서 3억 대출 변동금리 4.5% 30년 원리금균등으로 받으면 DSR이 몇 퍼센트야?",
    ...
    16: {
        "question": "서울(투기과열) 12억 아파트 무주택자인데, 9억 초과분은 LTV가 다르게 적용된다며? 정확히 계산해줘.",
        "answer": "9억까지 40% + 초과분(3억) 20% = 3.6억 + 0.6억 = 4.2억.",
        "key_facts": ["9억 이하 40%", "9억 초과 20%", "총 대출 한도 4.2억"],
    },
    17: {
        "question": "이미 집이 2채(다주택자)인데 서울(투기과열) 7억 아파트 주택담보대출 가능해? LTV 계산해줘.",
    ...
    20: {
        "question": "연소득 5천만이고 서울(투기과열)인데 금리 4.5% 30년 원리금균등이면 DTI 기준 최대 대출 가능액이 얼마야?",
        "answer": "연소득 7천만 이하 서민실수요자 DTI 60% 적용. 최대 대출 가능액 약 4.9억(49,340만원).",
        "key_facts": ["DTI 60%", "최대 대출 가능액 약 4.9억"],
    },
}
